<?php

namespace App\Policies;

use App\Models\Resep;
use App\Models\User;
use Illuminate\Auth\Access\Response;

class ResepPolicy
{
    /**
     * Determine whether the user can view any models.
     */
    public function viewAny(User $user): bool
    {
        return in_array($user->role, ['doctor', 'pharmacist']);
    }

    /**
     * Determine whether the user can view the model.
     */
    public function view(User $user, Resep $resep): bool
    {
        return in_array($user->role, ['doctor', 'pharmacist']);
    }

    /**
     * Determine whether the user can create models.
     */
    public function create(User $user): bool
    {
        return $user->role === 'doctor';
    }

    /**
     * Determine whether the user can update the model.
     */
    public function update(User $user, Resep $resep): bool
    {
        return $user->role === 'doctor';
    }

    /**
     * Determine whether the user can delete the model.
     */
    public function delete(User $user, Resep $resep): bool
    {
        return $user->role === 'doctor';
    }

    public function complete(User $user, Resep $resep): bool
    {
        return $user->role === 'doctor' && $resep->status === 'draft';
    }

    public function process(User $user, Resep $resep): bool
    {
        return $user->role === 'pharmacist' && $resep->status === 'completed';
    }

    /**
     * Determine whether the user can restore the model.
     */
    public function restore(User $user, Resep $resep): bool
    {
        return false;
    }

    /**
     * Determine whether the user can permanently delete the model.
     */
    public function forceDelete(User $user, Resep $resep): bool
    {
        return false;
    }
}
